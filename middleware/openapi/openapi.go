...
func New(config ...Config) fiber.Handler {
	...
	// Generated spec bytes are cached per spec target (a parameterized mount
	// can yield several) and invalidated whenever the app's route revision
	// changes. The same bound as the Swagger page cache applies.
	...
	var (
		specMu    sync.Mutex
		specPages = make(map[string]*specPage)
	)
	...
	return func(c fiber.Ctx) error {
		...
		case equal(request, specTarget):
			// The generated bytes are cached per spec target and invalidated by
			...
			cached, ok := specPages[specTarget]
			if !ok || cached.rev != rev {
				...
				cached = &specPage{rev: rev, data: data}
				if len(specPages) < maxCachedSwaggerPages || ok {
					specPages[specTarget] = cached
				}
			}
			data := cached.data
			specMu.Unlock()
